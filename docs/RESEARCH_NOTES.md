...
### P4-13 release workflow 与许可封存（2026-08-27，本地候选）
...
- 同一候选的 [Windows](https://github.com/song751/voice-trainer/actions/runs/33015196461) 已通过release、fake 6/6与P4-12 actual-target，最后在无保证音频输出端点的hosted runner上播放分支失败。集成gate仍无条件要求Rust提取与比较；只有显式CI环境变量存在时，typed `playbackFailed`可代表runner capability限制。本地无该变量，仍必须真实完成`NativeAudioPreview`窗口播放。修复后的hosted重跑尚未完成，不能把本段写作P4-13接受。
