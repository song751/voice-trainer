...
- 2026-08-27 最终复验使用 Edge 151 时发现浏览器会与 `about:blank` 同时创建 `edge://sync-confirmation-dialog/`；旧 CDP gate 只取首个 `page`，会附着到浏览器页面并产生空 sentinel。P4-09/P4-10/P4-11 现只选择 `about:blank` 或本项目 origin，冷启动等待保持有界 60 秒；P4-09/P4-10 也像 P4-11 一样核对 listener command line 后清理 Dart 子进程。P4-09 特殊 capture 构建补齐 `--no-web-resources-cdn`，避免测试 harness 与正式自包含部署合同分叉。修复后 permission deny、dedicated Rust worker/crash replacement、fake-media typed format failure、OPFS/IndexedDB reload/delete 及 lifecycle/deployment smoke 均重新通过。
...
### SRD-04 production 实现与平台证据（2026-08-27）

- production Rust 链路为：Symphonia mono/stereo decode → 44.1 kHz windowed-sinc resample → center-reflection periodic-Hann STFT 4096/hop 1024 → reviewed dynamic UMX-HQ magnitude core → mixture-phase vocals complex spectrum → complex residual accompaniment → normalized ISTFT。与 upstream Open-Unmix `Separator(niter=0, residual=true)` 对齐，不运行 Wiener，也不把 magnitude core 输出冒充 waveform。
- 模型分块为 300 frames、相邻 64-frame overlap，边界用线性权重归一化；短输入 pad 到至少 32 frames。确定性测试覆盖 STFT/ISTFT identity、residual invariant、长边界、重采样、分块交叉淡化、取消、模型缺失/哈希、原子双 stem、partial 清理、路径逃逸和 decoded-frame resource limit。
- 文件 job 输出 44.1 kHz stereo PCM16 WAV，以 `.partial` 写入、flush/sync 后 rename；任一失败清理双 partial/已提交单边文件。FRB 用 progress/terminal event stream，不阻塞采集队列或 UI；Flutter 先将 picker byte stream spool 到临时文件，再由 native worker 处理，并在 finally 清除临时输入/cancel marker。
- Flutter 模型由用户显式选择，最大 100 MiB，SHA-256 必须为 `1dd15a2be2f15ba035205f866a035df38d85b27824ad67fe53566e80ec1f4258` 才原子安装。模型/音频/stem 均不进 Git、不上传。typed failure 分列 rights、空/文件超限、unsupported/decode、model missing/hash、runtime/backend、resource、I/O、cancel 和 processing；输出 domain metadata 含 model/algorithm/source format/output format/chunk count 及双 stem locator/hash/byte length。
- Windows licensed 7 秒 product smoke：300,032 frames、1 chunk；vocals/accompaniment SHA-256 为 `9bec7249e1873b17d614a520c649f3dced67dcfa06e7a4b34fc1e69f260d9cc2` / `e1edc166931cf73f9e1cc5c805a83f14a040a3aa34df7e7cad678e193af3100c`。SRD-03 evaluator 得 vocals SI-SDR `12.287963 dB`、improvement `17.212332 dB`、residual `-125.942536 dBFS`，与 PyTorch oracle 的质量数值一致；仍只是单个 restricted research sample，不是听感接受。
- Windows x64 debug build 成功；`voice_trainer.exe` 1,267,200 bytes、Rust DLL 74,853,888 bytes。单次 warm-cache synthetic release smoke：30 秒 `1.134886 s`/7 chunks，3 分钟 `10.772957 s`/34 chunks，5 分钟 `7.724999 s`/56 chunks。后两次受 OS cache/并行构建影响，不能当严谨性能比较；Windows peak RSS 尚未量测。Windows decoded-frame 上限为 5 分钟。
- Android 四 ABI debug APK build 成功；主线 canonical release 另行实测 universal `118.7 MB`，split-per-ABI 为 armv7 `30.4 MB`、arm64 `41.2 MB`、x86_64 `48.0 MB`，这些包体均不包含 35.6 MB 外置模型权重。MuMu x86_64 模拟器 `127.0.0.1:16384`（明确不是性能真机）实际校验模型 hash、完成 tract graph compile，并经 Flutter picker/rights/FRB 对 1,200,206-byte WAV 生成两个 1,200,172-byte stem；UI 显示 ready。产物/model/输入随后清除，安装前 APK SHA-256 `a5aabc45b8e733dcd43b46a685c309b4451a39288699753f107e292c84c327fd` 已原样恢复。Android native capability 因而启用为 **probe-gated candidate**：只有固定 hash 模型在本设备成功 compile 后才 `available=true`，否则 typed unavailable；decoded-frame 上限暂为 1 分钟，等待真机 RSS/取消测试后才能放宽。
- Web debug build 成功，但 tract 不进入 WASM，`SongSeparator`/model manager 明确返回 unavailable；因此没有 Web 30 秒/3 分钟/5 分钟伪性能。若未来采用 Web runtime，必须单独通过 worker、单线程 fallback、模型许可/缓存、内存和浏览器矩阵 gate。

### P4-12 cross-platform UI/error/accessibility regression（2026-08-27，已完成，待集成接受）

- 统一 fixture 以同一 application/domain state 驱动 Windows、Android 与 Web profile，没有向页面加入平台判断。矩阵覆盖五主页面、歌曲导入、窄屏/宽屏、深色、200% 文字、触摸/鼠标/键盘/back，以及 permission/device/format/worker/recording/persistence/quality/completed/history/delete 状态。
...
### 分离人声 vs 练唱短句离线对比 MVP（2026-08-27）

- 新增 `reference-comparison-v1`：reference 使用独立 `reference-yin-14k7-v1` 提取 100 Hz pitch/RMS/periodicity，练唱侧直接读取既有版本化 packed `FeatureSeries`。对齐公开报告双方 onset、voiced-span tempo scale、原始 key difference 和最近整半音移调；pitch、timing、relative level envelope/stability、periodicity 分项显示，不合成总分。
- suppression-first：1–30 秒窗口、双方 voiced coverage、互相覆盖、练唱采集质量、reference clipping，以及用户试听确认的 artifact/monophonic lead 全部是硬门禁。失败只输出 coverage/flags/scope/原因；reference 始终标为模型分离估计，不能冒充真人标签。
- `audioplayers 6.8.1`（MIT）加入 production Flutter，用于 Windows/Android 应用内本地 WAV 窗口回放；上游同一 federated package 声明 Android/Web/Windows 等平台实现。本产品 Web reference runtime 当前 unavailable，因此不会伪装 Web A/B 可用。fallback 是 `AudioPreview` typed unavailable，可由测试 fake 注入；移除成本限于该 adapter/provider/UI 播放按钮，对 domain 对比和 Rust reference 特征不产生依赖。Windows plugin 要求 CMake policy 3.15，根 `windows/CMakeLists.txt` 因而从 3.14 提升到 3.15。许可与平台来源：<https://pub.dev/packages/audioplayers>、<https://github.com/bluefireteam/audioplayers>。
- 官方 `audioplayers_android 5.3.0` archive SHA-256 `f5ff5b15620fbab8cb0849e9636c48e2b96c3f0f71723bbbe2ad3c761b205f05` 固定在 `third_party/`。唯一 local patch 删除其嵌套 AGP 7.3.1/Kotlin 1.7.10/JUnit5 buildscript 与重复 repositories，复用仓库 AGP 9.0.1；Kotlin/Java 产品源码、MIT LICENSE/README/CHANGELOG 均保持上游。原因与 `file_selector_android` 相同：fresh build 否则会单独下载旧 AGP，且本机 Google Maven TLS 使该解析失败。`:audioplayers_android:assembleDebug --offline --rerun-tasks` 与完整四 ABI `flutter build apk --debug` 均实际通过；仍保留上游 manifest namespace 与 Android speakerphone API 的编译期 deprecation warning，不冒充 Android 播放真机验证。
- 本卡不新增 schema：song reference 当前未跨启动持久化，报告保持可重算的 application state，但显式携带 separator/reference/user/comparison 四个算法版本。`REFERENCE-AB-01` 继续为 `draft/unvalidated`；没有人工 reviewer 证据时不会伪造 `reviewed`。
- Windows 产品链 integration 使用运行时生成并清理的 3 秒、44.1 kHz stereo、220 Hz WAV，实际通过 Rust 文件 decoder/reference extractor 得到超过 250 帧、超过 240 voiced 帧且 A3 中位误差小于 2 cents；与高 200 cents 的 packed 练唱 series 对比后公开报告 `+2` 半音、对齐 pitch 中位误差小于 0.1 cents。production `NativeAudioPreview` 也实际播放 0.10–0.25 秒窗口并停止。该证据验证进程内 Windows 链路，不是听感或扬声器质量结论。
- 构建/静态 gate：Windows debug、自包含 Web release、四 ABI Android debug、Flutter analyze/146 项 unit-widget、Rust fmt/Clippy/34 项测试均通过；reference/comparison 新增的 deterministic tests 覆盖对齐、质量抑制、UI contract 和 vendor audit。Android 未连接 emulator/真机，因此 build 通过不冒充播放、温升、内存或生命周期证据；Web 继续 typed unavailable。

### P4-08 Android emulator release 闭环（2026-08-27，已完成，待集成接受）

- evidence runner 固定 SDK ADB endpoint `127.0.0.1:16384`，实际 profile 为 API 35、x86_64、1080×1920@480。release APK 真实执行 permission denied→granted；随后 synthetic PCM 才进入 production Rust/WAV/Drift 链路，因此权限是 emulator plugin/platform evidence，其余声音内容是 synthetic，不是 real-microphone 或真人声证据。root 未使用。
- 首次 600 秒 run 精确送入 28,800,000 samples 且无 native crash，但在 final repository save 暴露暂停/恢复后的合法 sample-index gap 无法被等间隔 metadata 表示。修复没有占用数据库 schema 6：规则 timeline 仍是 feature schema v1/v2；不规则 timeline 才用 v3 `sample_index_u64` 单列，payload 含 magic/version/frame count 与小端 uint64 absolute indices，并由 SHA-256 保护。读取保留 v1/v2 arithmetic timeline；v3 校验长度、checksum、单调性、首索引与跨 Web 精确整数范围。高起始索引/gap/bands round-trip 测试及带 pause/HOME 的 15 秒 release save/reload/delete 均通过。
- 第二次正式 release run 在 commit `ff2579dbac96` 完整通过 600.000 秒 active / 605.201 秒 wall、28,800,000 samples、2 discontinuities、0 analysis/recording queue drops、primary worker/0 restart。pipeline P95 `68.535 ms`，UI build/raster P95 `0.666/1.405 ms`；21 个 memory samples 的 PSS `80.855–95.527 MiB`、RSS `166.484–183.223 MiB`，没有将 emulator 数字解释为真机性能门槛。
- universal release APK 包含 x86_64 Rust library，大小 `117,546,074` bytes，SHA-256 `eb95083e242a6ab07ae8792cfc96ef64d46879b68f993a5777f61f0d40d85909`。本地 ignored bundle 通过 strict validator；结果/历史/录音删除、force-stop/relaunch 后 history reload/session delete 均由 sentinels 验证。runner finally 后测试 package 与临时状态文件均不存在。
- 真实 Android 麦克风、硬件 route、AGC、latency、真人 voiced input、来电/蓝牙以及 Android 真机 10 分钟稳定性仍为 Pending；本卡不关闭 P3-07D、不解锁 P4-14，也不构成“Android 已支持”。

### Reference comparison 内容身份与 locator hardening（2026-08-27）

- Drift schema v6 为 `recordings` 和 `feature_series_metadata` 各增加 nullable SHA-256/byte-length；Native/Web finalize 计算新 WAV 内容身份，coordinator 在 finalize 后绑定 packed provenance。v5 及更早记录保持 NULL，严禁从当前文件回填，因而不会把已替换文件误配给历史指标。P4-08 的 feature schema v3 `sample_index_u64`/不规则 timeline 同时保留。
- Native 新记录只持久化 opaque basename。统一 managed-root resolver 对 opaque locator 与 legacy absolute locator 做 canonical containment；legacy 只允许 root 内 exists/delete，越界、路径逃逸、missing、length/hash mismatch 都返回 typed failure。验证会复制到私有 snapshot 后再核对长度/hash，extractor/player 只接 lease path，源文件在验证后被替换不影响本次输入，lease 销毁即清理。
- separator 在发布 ready reference 前再次核验 Rust 报告的 stem 位于本次 managed output root 且长度/hash 一致；domain reference engine 还会强制 `verified stem == SongStemReference == ReferenceAnalysisSeries` 与 `verified recording == RecordingLocator == FeatureSeries`。任一缺失/不匹配只产生 metric-free suppression 和 `REC-QUALITY-01`，不产生 `REFERENCE-AB-01`；UI 只显示 12 位短内容 ID，不输出完整 hash/路径。
- Web BlobStore/OPFS/IndexedDB locator 与 JS API 未变化，仅在已有 Dart WAV bytes 上计算内容身份；native resolver 在 Web 明确 typed unavailable。没有新增 production dependency。
- P4-12 的 Windows/Android/Web dark + 200% fixture 已加入 reference route empty state，以及 fake-ready 从人工门禁 suppression 到可解释 metrics 的 scroll/semantics/back 合同。该测试仍是 synthetic profile evidence，不是 Android 真机或 Web reference runtime 通过。
- 实际 gate：Drift codegen 二次运行的 generated hash 不变；Flutter format/analyze 与 199 项全量 test、Rust fmt/Clippy/全部非 ignored tests 通过。Windows actual-target integration 实际完成 Rust reference 提取、双边绑定比较和 `NativeAudioPreview` 窗口播放，P4-12 actual-target route contract 也通过；自包含 CSP Web release 与四 ABI universal Android debug build 成功。Android 仅为 build evidence，本卡未连接模拟器/真机，Web reference runtime 仍 typed unavailable。

### P4-13 release workflow 与许可封存（2026-08-27，接受）

- GitHub官方Node 20弃用计划要求迁移Node 24 action；直接action采用完成迁移后的最小major并锁完整SHA：checkout v5.0.1、setup-java v5.7.0、upload-artifact v6.0.0、setup-android v4.0.1。Flutter action v2.23.0与dtolnay生成的Rust 1.97.1 action也锁commit；版本tag保留供复核。`subosito/flutter-action`是composite且源码内部仍引用移动`actions/cache@v5`；四个workflow均显式设置`cache: false`，使对应条件分支不可达，release audit也强制这一约束。若未来需要恢复action cache，必须先vendor/fork并锁定其传递action。官方依据：<https://github.blog/changelog/2025-09-19-deprecation-of-node-20-on-github-actions-runners/>、<https://docs.github.com/en/actions/reference/security/secure-use#using-third-party-actions>及各action官方release。
- FRB 2.12 `build-web`会覆盖外层`RUSTUP_TOOLCHAIN`，因此使用正式参数 `--wasm-pack-rustup-toolchain nightly-2026-08-02`；日志确认rustc 1.99.0-nightly commit `73dc9167f1cd099e525c9ade2e068d1907b78564`与rust-src来自该日期toolchain。
- license audit覆盖production与song-separation两份locked Cargo graph。Symphonia 0.5.5为MPL-2.0并记录精确crate archive/checksum及上游tag；tract系列为MIT OR Apache-2.0；vendored audioplayers Android为MIT、file_selector Android为BSD-3-Clause。模型、用户音频和stem不进入Git/CI artifact。
- Windows/Android artifact在binary旁上传NOTICE/vendor license；Web将其复制到deployment并把NOTICE纳入critical release manifest。artifact尚无签名、SBOM、attestation或正式GitHub Release，本卡不把普通CI称作公开发布管线。
- 本地候选已通过Dart/Rust/song-tool gates、Windows release及三条actual-target integration、固定nightly FRB WASM、自包含Web release与部署validator。Android universal release APK为126,789,570 bytes，SHA-256 `e43fc7fa129a7693b322257d0799d71407a9134fa19209aacf6dc10360a1ab5e`，外置模型不在包内。hosted CI、最终emulator 600秒bundle、合入后只读审计仍Pending；P3-07D、P4-14、P4-15及Closure未完成。
- 首次最终候选 `8213bd9` 的 hosted [Web](https://github.com/song751/voice-trainer/actions/runs/33015196451) 与 [Dart/Rust](https://github.com/song751/voice-trainer/actions/runs/33015196579) 通过；[Android](https://github.com/song751/voice-trainer/actions/runs/33015196420) 暴露 Flutter 3.44.7 的已知 `--no-pub` release registrant 回归：生成文件仍引用 dev-only `integration_test`，Gradle release classpath则正确排除它。采用Flutter上游issue #169336的最小绕法，让release命令自行刷新pub/plugin配置；release audit拒绝再次加入该flag，没有把测试插件移入production dependency。
- 同一候选的 [Windows](https://github.com/song751/voice-trainer/actions/runs/33015196461) 已通过release、fake 6/6与P4-12 actual-target，最后在无保证音频输出端点的hosted runner上播放分支失败。集成gate仍无条件要求Rust提取与比较；只有显式CI环境变量存在时，typed `playbackFailed`可代表runner capability限制。本地无该变量，仍必须真实完成`NativeAudioPreview`窗口播放。
- 修复候选 `b221014` 的 hosted [Dart/Rust](https://github.com/song751/voice-trainer/actions/runs/33017279397)、[Windows](https://github.com/song751/voice-trainer/actions/runs/33017279382)、[Android](https://github.com/song751/voice-trainer/actions/runs/33017279406) 与 [Web](https://github.com/song751/voice-trainer/actions/runs/33017279356) 均completed successfully；release artifact与NOTICE上传成功。结合最终竖屏emulator 600秒bundle和合入后零blocker/high审计，P4-13接受并完成。真实设备、真实浏览器麦克风、P3-07D与P4-16 Closure仍保持人工Gate。
