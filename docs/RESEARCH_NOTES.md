...
- GitHub官方Node 20弃用计划要求迁移Node 24 action；直接action采用完成迁移后的最小major并锁完整SHA：checkout v5.0.1、setup-java v5.7.0、upload-artifact v6.0.0、setup-android v4.0.1。Flutter action v2.23.0与dtolnay生成的Rust 1.97.1 action也锁commit；版本tag保留供复核。`subosito/flutter-action`是composite且内部仍引用移动`actions/cache@v5`，严格传递依赖全锁需未来vendor/fork；当前不隐藏该残余风险。官方依据：<https://github.blog/changelog/2025-09-19-deprecation-of-node-20-on-github-actions-runners/>、<https://docs.github.com/en/actions/reference/security/secure-use#using-third-party-actions>及各action官方release。
