...
当前状态：**Phase 1 与 Closure C1–C4、P2-01 至 P2-07，以及 Phase 3 的 P3-00 至 P3-06、工作树 checkpoint `P3-07P` 和证据合同 `P3-07A` 均已完成并接受**。当前执行远程准备卡 `P3-07B`，随后才是 `P3-07C`。若这些卡完成且仓库所有者仍不在电脑旁，可以按授权的 P4-00→P4-13 路径依次推进 Android 模拟器、跨平台 UI 和 Web 实现。P3-07D、Android 真机、真实浏览器麦克风和最终 Closure 仍明确阻塞，任何模拟器/root/fake 结果都不能替代。
...
按 [`docs/PHASE3_TASKS.md`](docs/PHASE3_TASKS.md) 当前只执行 `P3-07B`，然后执行 `P3-07C`。之后若仍无法现场操作，进入 [`docs/PHASE4_TASKS.md`](docs/PHASE4_TASKS.md) 的 `P4-00`，按卡推进到 `P4-13`；回到电脑旁后在已接受卡边界切回 `P3-07D`。不得并行混合两张卡，也不得把 MuMu 模拟器证据写成 Android 真机通过。
