...
当前状态：**P4-00 至 P4-13 的远程实现基线已接受；Windows/Web/Android release、四条 hosted CI、竖屏 emulator 600 秒 bundle 与零 blocker/high 审计均已通过**。Android 默认使用 `record` + native Rust worker + native Drift/WAV persistence；Web 默认使用 `record_web` + dedicated single-thread Rust WASM worker、Drift shared IndexedDB 与 OPFS recording store，并有自包含 CSP deployment gate。歌曲分离在 Windows/Android native 使用 Git 外固定哈希模型与 typed capability probe，Web 明确 unavailable。`P3-07D`、P4-14 Android 真机、P4-15 真实浏览器/麦克风及 P4-16 Closure 仍 Pending，任何 build、模拟器、root、fake 或 synthetic 结果都不能替代。
...
远程可完成的 P4-13 基线已经封存。下一步只能进入人工/真实设备门槛：P3-07D 仍须仓库所有者在 Windows 工作站旁执行；P4-14 需要 Android 真机与真实麦克风/路由，P4-15 需要真实浏览器与麦克风，之后才可执行 P4-16 Closure。
