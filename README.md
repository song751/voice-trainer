...
当前状态：**Phase 1 Closure，C1「建立可信仓库基线与文档同步」和 C2「关闭 Worker gate」已完成**。本地基线、忽略规则、生成物策略、源码范围 format gate、GitHub remote 与首次 hosted CI 证据均已建立；真实 Edge dedicated worker 已与 Windows FRB 完成确定性对比。首轮 Web generated-file drift 已修复并重新触发；全部 hosted CI 绿灯仍由 C4 统一验收，不能提前解锁 Phase 2。
...
继续按 [`docs/PHASE1_CLOSURE_PLAN.md`](docs/PHASE1_CLOSURE_PLAN.md) 完成 C3–C4；当前允许进入 C3。真实麦克风覆盖继续按 [`docs/test-matrices/`](docs/test-matrices/) 中的平台矩阵进行；不要用 fake capture、emulator 或构建成功冒充真实设备通过。
